...
type Tracer interface {
	...
	SetError(new_err error, pos int)
}
...
func (l *StructLogger) SetError(new_err error, pos int) { // required by EthBot to catch errors in internal transactions between contracts
	if (pos>-1) {
		l.logs[pos].Err=new_err
	}
}
